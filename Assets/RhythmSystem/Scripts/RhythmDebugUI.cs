...
bool IsInputCycle(int cycle)
{
    ...
    public void ResetVisual()
    {
        ...
        // reset colors if circles exist
        for (int i = 0; i < 3; i++)
        {
            if (_circleImg[i] != null) _circleImg[i].color = defaultColor;
            if (_circleRT[i] != null) _circleRT[i].localScale = Vector3.one;
            if (_dotRT[i] != null) _dotRT[i].gameObject.SetActive(false);
        }
    }
